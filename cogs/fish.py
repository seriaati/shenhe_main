            ...
            if fish["type_0"]:
                if value <= int(fish["flow_chance"]):
                    ...
                    await interaction.followup.send(
                        f"摸**{self.fish_adj}的{self.fish}**摸到 {flow} flow幣!\n目前 flow 幣: {await get_user_flow(interaction.user.id, self.db)}",
                        ephemeral=True,
                    )
                    # e.g. 摸虱目魚摸到 1 flow幣!
                else:
                    await interaction.followup.send(
                        f"單純的摸**{self.fish_adj}的{self.fish}**而已, 沒有摸到flow幣 qwq\n目前 flow 幣: {await get_user_flow(interaction.user.id, self.db)}",
                        ephemeral=True,
                    )
            else:
                chance = 40
                ...
                if value <= chance:  # 50% Chance of increasing flow amount by 20
                    ...
                    await flow_transaction(interaction.user.id, -int(flow), self.db)
                    await interaction.followup.send(
                        f"被**{self.fish_adj}的{self.fish}**{random.choice(verb)}，損失了 {flow} flow幣 qwq\n目前 flow 幣: {await get_user_flow(interaction.user.id, self.db)}",
                        ephemeral=True,
                    )
