...
class RollApp:
    ...
    async def gu_system(self, user_id: int, is_ten_pull: bool):
        ...
        await c.execute(
            "SELECT SUM(count) FROM roll_guarantee WHERE user_id = ?", (user_id,)
        )
    ...
    async def check_big_prize(self, user_id: int, prizes: list[str]):
        if banner["big_prize"]["name"] in prizes:
            c = await self.db.cursor()
            await c.execute(
                "UPDATE roll_guarantee SET count = 0 WHERE user_id = ?", (user_id,)
            )
    ...
    async def write_history_and_gu(self, user_id: int, prizes: list[str]):
        ...
        for prize in prizes:
            await c.execute(
                "INSERT INTO roll_history (user_id, prize, count) VALUES (?, ?, 1) ON CONFLICT (user_id, prize) DO UPDATE SET count = count + 1 WHERE user_id = ?",
            ...
            if prize != banner["big_prize"]["name"]:
                await c.execute(
                    "INSERT INTO roll_guarantee (user_id, prize, count) VALUES (?, ?, 1) ON CONFLICT (user_id, prize) DO UPDATE SET count = count + 1 WHERE user_id = ?",
                    (user_id, prize, user_id),
                )
            prize_str += f"• {prize}\n"
        if air_count == 10:
