...
class FlowApp:
    ...
    async def transaction(
        ...
        is_removing_account: bool = False,
    ):
        now = datetime.now()
        time_states = ["morning", "noon", "night"]
        if is_removing_account:
            log(True, False, "Removing Acc", f"{user_id}: (flow = {flow_for_user})")
        ...
        if is_new_account:
            ...
            await c.execute(
                f"UPDATE flow_accounts SET flow = ? WHERE user_id = ?", (0, user_id)
            )
            for time in time_states:
                await c.execute(
                    f"UPDATE flow_accounts SET {time} = ? WHERE user_id = ?",
        ...
        await c.execute(
            "UPDATE flow_accounts SET flow = ? WHERE user_id = ?",
            (user_flow + flow_for_user, user_id),
        )
        bank_flow = await self.get_bank_flow()
        ...
        await c.execute(
            "UPDATE flow_accounts SET last_trans = ? WHERE user_id = ?", (now, user_id)
        )
        await self.db.commit()
        log(
            True, False, "Transaction", f"user({user_id}): {user_log}, bank: {bank_log}"
        )

    async def checkFlowAccount(self, user_id: int) -> Union[bool, Embed]:
        c = await self.db.cursor()
        await c.execute(
            "SELECT user_id FROM flow_accounts WHERE user_id = ?", (user_id,)
        )
        result = await c.fetchone()
        if result is None:
            await self.register(user_id)
            embed = errEmbed("找不到flow帳號!", f"<@{user_id}>\n現在申鶴已經創建了一個, 請重新執行操作")
            return False, embed
        else:
            return True, None
